...
```{r}
prescribed_medicine <- data_march2024 %>% 
  group_by(bnf_item_description) %>% 
  summarise(total_presciption = sum(paid_quantity)) 
most_prescribed_medicine <- prescribed_medicine %>% 
  slice_max(total_presciption , n = 10) %>% 
  gt()
...
```{r}
HBN <- read.csv("https://www.opendata.nhs.scot/dataset/9f942fdb-e59e-44f5-b534-d6e17229cc7b/resource/652ff726-e676-4a20-abda-435b98dd7bdc/download/hb14_hb19.csv") %>%
  clean_names()
joined_data_march2024 <- data_march2024 %>% 
  full_join(HBN, join_by(hbt == hb))
...
```{r}
na_joined_data_march2024 <- joined_data_march2024 %>% 
...
```{r}
joined_data_march2024 <- joined_data_march2024 %>% 
...
```{r}
paracetamol_data <- joined_data_march2024 %>% 
...
```{r}
paracetamol_capsules_per_person <- paracetamol_data %>% 
  
  full_join(population_data) %>% 
  filter(str_detect(bnf_item_description, "CAP")) %>%  
  group_by(hb_name) %>% 
  summarise(capsules_per_person = (sum(paid_quantity)/mean(hb_population)))

paracetamol_plot <- paracetamol_capsules_per_person %>% 
  ggplot(aes(x = reorder(hb_name,-capsules_per_person), y = capsules_per_person)) +
  geom_col() +
  theme(
...
```{r}
peak_pandemic_para <- read.csv("data/pitc202203.csv")
